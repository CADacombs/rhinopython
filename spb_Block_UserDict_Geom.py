...
This script uses UserDictionary of a block definition to
    1. Store (w, a) duplicates of geometry of objects with option to delete the source DocObjects.
    2. Retrieve (r) geometry and add them to the document with option to delete
    the source geometry in the block definitions (rd).
    ...
...
def _store_in_def_of_ref_Geoms_of_objrefs(rgIref, objrefs_In, sKey, bAppend=True, bEcho=True, bDebug=False):
    """
    Returns: bool for Success
    """

    geoms_ToStore = List[rg.GeometryBase]([o.Geometry() for o in objrefs_In])
...
def _get_keys_for_geometries(rgIref_or_rdIdef):
    ...
    if isinstance(rgIref_or_rdIdef, rd.InstanceDefinition):
        ...
        raise Exception("{} sent to get_keys_with_geometric_values.".format(
