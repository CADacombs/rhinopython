...
Rhino 7.15.22039.13001 on Windows
...
2. FreeCAD, preferrably the same version as stated above,
    ...
    installed where the Rhino application instance running the script can access it.
...
TODO:
Prepare default paths easier setup for Mac.
Maybe?: Allow variable size chamfer input.
...
"""
220220: Created, starting with the likewise 'FilletEdge' script.
"""
...
class Opts:
    ...
    key = 'fChamferDist'; keys.append(key)
    values[key] = 1.0
    names[key] = 'Distance'
...
def getInput(bPrevBrepsArePresent):
    ...
    go.SetCommandPrompt("Select edges to chamfer")
...
def create_subBs_per_Es_to_chamfer(rgB, idxs_Es):
    """
    """
    ...
    # of the chamfers are processed correctly.
    iEs_NoChamfer = []
    for iF in iFs:
        for iE in rgB.Faces[iF].AdjacentEdges():
            if iE in idxs_Es: continue
            if iE in iEs_NoChamfer: continue
            iEs_NoChamfer.append(iE)

    for iE in iEs_NoChamfer:
    ...
    iEs_SharedVs = []
    for iE_Chamfer in idxs_Es:
        v = rgB.Edges[iE_Chamfer].StartVertex
        for iE in v.EdgeIndices():
            if iE in idxs_Es: continue
            if iE in iEs_NoChamfer: continue
            iEs_SharedVs.append(iE)
        v = rgB.Edges[iE_Chamfer].EndVertex
        for iE in v.EdgeIndices():
            if iE in idxs_Es: continue
            if iE in iEs_NoChamfer: continue
            iEs_SharedVs.append(iE)
...
def create_FC_code_Determine_edges_to_chamfer():
    ...
    fc()
    fc('edges_to_chamfer = []')
    fc('idx_edges_to_chamfer = []')
    fc('chamfer_dists_per_idxE = []')
    fc()
    fc('for iE, edge in enumerate(doc.Chamfer.Base.Shape.Edges):')
    ...
    fc('        continue')
    fc('    edges_to_chamfer.append(edge)')
    fc('    idx_edges_to_chamfer.append(iE+1)') # Part Edge indices are base 1 in FreeCAD.
    fc('    chamfer_dists_per_idxE.append(rads_per_pts_Mids[idxMid])')
    fc('    if len(edges_to_chamfer) == len(midVs):')
    fc('        break')
    fc('else:')
...
def subprocess_FC(sArgs, fFreeCADTimeoutSecs=5.0, bEcho=True):
    ...
    try:
        ...
        if stderr or p.returncode:
            if bEcho:
                print("Failed to get FreeCAD to complete the chamfering.")
...
def processBrep(rgBrep_In, idxs_Edges, fRadii, fFreeCADTimeoutSecs=10.0, bEcho=True, bDebug=False):
    ...
    rgBs_ToChamfer, idx_Fs_to_FC = create_subBs_per_Es_to_chamfer(rgBrep_In, idxs_Edges)

    if not rgBs_ToChamfer:
        if bEcho:
            print("No shells to chamfer in a brep.")
        return False, None
    ...
    for rgB_ToChamfer in rgBs_ToChamfer:

        Rhino.RhinoApp.SetCommandPrompt("Exporting STEP ...")

        if not export_to_STEP(rgB_ToChamfer, bDebug): return
        ...
        fc('doc.addObject("Part::Chamfer","Chamfer")')
        fc('doc.Chamfer.Base = doc.Shape')
        fc('doc.recompute()')


        create_FC_code_Determine_edges_to_chamfer()


        # Chamfer.
        fc('data_for_featChamfer = []')
        fc('for i, iE in enumerate(idx_edges_to_chamfer):')
        fc('    data_for_featChamfer.append((iE, chamfer_dists_per_idxE[i], chamfer_dists_per_idxE[i]))')
        fc()
        fc('doc.Chamfer.Edges = data_for_featChamfer')
        fc('doc.recompute()')
    ...
    fc('import Import')
    fc('Import.export([FreeCAD.ActiveDocument.Chamfer], r"{}")'.format(sPath_STEP_from_FC))
...
def processBrepObject(rhBrep_In, idxs_Es, fRadii, fFreeCADTimeoutSecs=10.0, bEcho=True, bDebug=False):
    ...
    if not rdBs_from_FC:
        if bEcho:
            print("Chamfered breps were not imported.")
        return
...
def main():
        ...
        for rhObj in rhObjs_In:
            ...
            if gB in gBs_In:
                if idxE in idx_rgEdges_PerBrep[gBs_In.index(gB)]:
                    ...
                    rads_for_idxEs_PerBrep[gBs_In.index(gB)][idxEperB] = fChamferDist
                    continue

                idx_rgEdges_PerBrep[gBs_In.index(gB)].append(idxE)
                rads_for_idxEs_PerBrep[gBs_In.index(gB)].append(fChamferDist)
                continue

            gBs_In.append(gB)
            idx_rgEdges_PerBrep.append([idxE])
            rads_for_idxEs_PerBrep.append([fChamferDist])
    ...
    while True:
        ...
        fChamferDist = Opts.values['fChamferDist']
        ...
        if not rc:
            print("Proceeding to create chamfers.")
            conduit.Enabled = False
            del conduit
            break
