...
def getInput_Grips_ToMove():
    ...
    go.SetCommandPrompt("Select grips/CPs to move")
...
def getInput_Points_Ref():
    ...
    for i, s in zip((0,1),('1st', '2nd')):
        ...
        gp.SetCommandPrompt("Select {} reference point".format(s))
...
def processGrips(grips_ToMove, pts_Ref, bDistributeEvenly=False, bEcho=True, bDebug=False):
    """
    """
    ...
    if bDistributeEvenly:
        ...
        for i in range(len(grips_ToMove_Sorted)):
            perunum = float(i)/float(len(grips_ToMove_Sorted)-1.0)
